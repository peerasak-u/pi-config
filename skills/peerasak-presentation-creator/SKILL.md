---
name: peerasak-presentation-creator
description: Create data-driven presentation slides using React, Vite, and Recharts with custom Slop Engineer branding. Use when asked to "create a presentation", "build slides", "make a deck", "create a data presentation", "build a presentation". Scaffolds a complete slide-based app with charts, animations, and single-file HTML output.
---

# Slop Engineer Presentation Builder

Create interactive, data-driven presentation slides using React + Vite + Recharts, styled with the custom earth-brown design system and built as a single distributable HTML file.
...
└── src/
    ...
    ├── App.css
    ├── Charts.jsx
    ├── slop-engineer-logo.svg
    └── slop-engineer-glyph.svg
```
...
### Copy SVG Reference Files

Copy the official SVG files from `${CLAUDE_SKILL_ROOT}/references/` into `src/`:
- `slop-engineer-logo.svg` (full wordmark for title slides)
- `slop-engineer-glyph.svg` (glyph only for watermarks)

Use Vite's `?raw` import to avoid token bloat from inline SVG paths:

```jsx
import SlopEngineerLogoSvg from './slop-engineer-logo.svg?raw';
import SlopEngineerGlyphSvg from './slop-engineer-glyph.svg?raw';

// ── Logo Components (from reference files) ──────────────────────────────────
function SlopEngineerLogo({ width = 160 }) {
  return (
    <svg viewBox="0 0 988 155" width={width} fill="none" aria-hidden="true" dangerouslySetInnerHTML={{ __html: SlopEngineerLogoSvg }} />
  );
}

function SlopEngineerGlyph({ size = 44 }) {
  return (
    <svg viewBox="0 0 1395 1150" width={size} height={size} aria-hidden="true" dangerouslySetInnerHTML={{ __html: SlopEngineerGlyphSvg }} />
  );
}
```
...
Read `${CLAUDE_SKILL_ROOT}/references/design-system.md` for the complete earth-brown color palette, typography, CSS variables, layout utilities, and animation system.
...
function App() {
  ...
  return (
    <>
      {cur > 0 && <div className="glyph-watermark"><SlopEngineerGlyph size={50} /><span className="watermark-title">TITLE</span></div>}
...
## Step 5: Style with Custom Design System

Apply the complete CSS from the design system reference. Key elements:

- **Font**: Google Sans from Google Fonts (supports Thai)
...
- **Logo**: Use the SVG files copied to `src/slop-engineer-logo.svg` and `src/slop-engineer-glyph.svg` with the `?raw` import pattern. Do NOT hardcode SVG paths inline.

## Step 6: Common Slide Patterns

### Title Slide
`<SlopEngineerLogo />` + h1 + subtitle + author/date info.
...
- Uses custom earth-brown branding (colors, fonts, icons)
