...
:root {
  /* ── UI chrome ── */
  --dark: #3D2B00;
  --brand: #594100;      /* primary brand color (earth brown) */
  --brand-light: #A67B5B;
  --brand-bg: #F0E6D2;
  --bg: #F7F4EE;
  --card: #F2ECD8;
...
// ── Categorical palette (Earth brown inspired) ──
...
**Font**: Google Sans (Google Fonts) with system-ui fallback. Supports Thai.
...
.progress {
  ...
  background: var(--brand); transition: width 0.3s; z-index: 10;
}
...
Every slide shows a small, low-contrast Slop Engineer glyph in the top-left corner. Use the exact path from `${CLAUDE_SKILL_ROOT}/references/slop-engineer-glyph.svg`.
...
**Do NOT hardcode the logo as inline SVG.** Read the official SVGs from the skill references directory:

- **Full wordmark**: `${CLAUDE_SKILL_ROOT}/references/slop-engineer-logo.svg` — the "Slop Engineer" logotype with glyph. Use on title slides.
- **Glyph only**: `${CLAUDE_SKILL_ROOT}/references/slop-engineer-glyph.svg` — the standalone glyph mark. Use for compact branding.
...
function SlopEngineerLogo({ width = 120 }) {
  // The SVG content should be read from references/slop-engineer-logo.svg
...
function SlopEngineerGlyph({ size = 32 }) {
  return (
